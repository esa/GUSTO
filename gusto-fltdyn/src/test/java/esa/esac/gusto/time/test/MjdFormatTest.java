...
public class MjdFormatTest {
	...
	MjdTimeFormat converter = new MjdTimeFormat(TimeScale.UTC);

	String s = "1999-01-01T00:00:00Z";
	TaiTime f0 = utcFormat.parse(s);
	final double mjd0 = converter.TaiTimeToMjd(f0);
	assertEquals(mjd0, 51179.0, 1E-14);
	{
	    // Test 1998-12-31T23:59:59Z
	    TaiTime ft = f0.addMicroseconds(-2000000);
	    double mjd = converter.TaiTimeToMjd(ft);
	    assertEquals(mjd + sec, mjd0, EPS);

	    TaiTime f = converter.mjdToTaiTime(mjd);
	    ...
	    double mjd = converter.TaiTimeToMjd(f0);
	    assertEquals(mjd, mjd0, EPS);
	...
	MjdTimeFormat ttFormat  = new MjdTimeFormat(TimeScale.TT);
	MjdTimeFormat taiFormat = new MjdTimeFormat(TimeScale.TAI);

	String s1 = ttFormat.format(ft);
	assertEquals(s1, "51179.0003725"); // 51179 + 31.184 / 84000
	TaiTime ftp = ttFormat.parse(s1);
	assertEquals(ft.microsecondsSince1958(), ftp.microsecondsSince1958());

	double mjdTt  = ttFormat.TaiTimeToMjd(ft);
	double mjdTai = taiFormat.TaiTimeToMjd(ft);
	assertTrue(Math.abs(mjdTt - mjdTai - 32.184 / 86400d) < EPSILON);

	TaiTime ft2 = ttFormat.mjdToTaiTime(mjdTt);
	assertEquals(ft.microsecondsSince1958(), ft2.microsecondsSince1958());
    }
