...
/**
 * A pane that displays a Schedule as a time-line.<p>
 * 
 * This is just a simple schedule with fixed observations for the purposes of the example.
 *
 * @author  Jon Brumfitt
 */
class DemoSchedulePane extends JPanel implements ChangeListener {
     ...
     */
    public DemoSchedulePane(BoundedTimeModel model, TimeProjection projection) {
	_model = model;
	...
	g2.fill(new Rectangle2D.Float(xs, 0, xe - xs, 28));
	...
	for(TimeInterval item : _items) {
	    ...
	    g2.fill(new Rectangle2D.Float(x1, 5, x2 - x1, 18));
	}
    }
