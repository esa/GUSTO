...
public class OrthographicProjectionTest {
	...
	SkyProjection proj = new OrthographicProjection(q1);
	...
	SkyProjection p = new OrthographicProjection();
	assertTrue(p.getName().equals("Orthographic"));
    }
    
    @Test
    public void testDefaultViewport() {
	SkyProjection proj = new OrthographicProjection();
	double scale = 3;
	proj.setScale(scale);
	...
	assertEquals(v1n.getY() * scale, -p1.getX(), 1E-14);
	assertEquals(v1n.getZ() * scale, p1.getY(), 1E-14);
	
	// Test inverse
	Vector3 r1 = proj.inverse(p1);
	assertTrue(r1.epsilonEquals(v1n, 1E-15));

	// Forward projection limiting case
	Vector3 v2 = new Vector3(0,2,3)	;
	Vector3 v2n = v2.normalize();
	Vector2 p2 = proj.forward(v2);
	assertEquals(v2n.getY() * scale, -p2.getX(), 1E-14);
	assertEquals(v2n.getZ() * scale, p2.getY(), 1E-14);
	
	// Test inverse in limiting case
	Vector3 r2 = proj.inverse(p2);
	assertTrue(r2.epsilonEquals(v2n, 1E-15));
	
	// Hidden part of sphere returns null
	Vector3 v3 = new Vector3(-0.00001,2,3);
	...
	SkyProjection d = new OrthographicProjection();
	String s = d.toString();
	assertFalse(s == null);
